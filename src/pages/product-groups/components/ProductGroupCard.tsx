/**
 * Single product group card component
 */

import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import type { AirtableRecord, ProductGroupFields } from '@/lib/airtable-api'
import { formatCreatedDate } from '../product-groups-utils'

interface ProductGroupCardProps {
  productGroup: AirtableRecord<ProductGroupFields>
  isSelected: boolean
  isToggling: boolean
...
export function ProductGroupCard({
  ...
  // Debug: Log product group data to verify Status field
  if (process.env.NODE_ENV === 'development') {
    console.log('[ProductGroupCard] Product Group:', productGroup.fields.Name, 'Status:', productGroup.fields.Status, 'All fields:', Object.keys(productGroup.fields))
  }

  // Get status with fallback - check for different possible field names/cases
      ...
      <CardHeader className="pb-4 pt-6 px-6">
        <div className="flex items-start gap-3">
          <input
            type="checkbox"
            checked={isSelected}
            onChange={() => onSelect(productGroup.id)}
      ...
      <CardContent className="pt-0 px-6 pb-6">
        <div className="flex items-center justify-between mb-4 pt-4 border-t border-border">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                onToggleActive(productGroup.id, status)
              }}
              disabled={isToggling}
        ...
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span className="text-xs">
            Created {formatCreatedDate(productGroup.createdTime)}
          </span>
        </div>
      </CardContent>
    </Card>
