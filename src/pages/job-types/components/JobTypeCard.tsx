/**
 * Single job type card component
 */

import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import type { AirtableRecord, JobTypeFields } from '@/lib/airtable-api'
import { formatCreatedDate } from '../job-types-utils'

interface JobTypeCardProps {
  jobType: AirtableRecord<JobTypeFields>
  isSelected: boolean
  isToggling: boolean
...
export function JobTypeCard({
  ...
  // Debug: Log job type data to verify Status field
  if (process.env.NODE_ENV === 'development') {
    console.log('[JobTypeCard] Job Type:', jobType.fields.Name, 'Status:', jobType.fields.Status, 'All fields:', Object.keys(jobType.fields))
  }
            ...
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0 pr-8">
                <CardTitle className="text-lg font-semibold break-words leading-tight text-foreground">
                  {jobType.fields.Name || 'Unnamed Job Type'}
                </CardTitle>
              </div>
            </div>
          </div>
      ...
      <CardContent className="pt-0 px-6 pb-6">
        <div className="flex items-center justify-between mb-4 pt-4 border-t border-border">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                onToggleActive(jobType.id, status)
              }}
              disabled={isToggling}
        ...
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span className="text-xs">
            Created {formatCreatedDate(jobType.createdTime)}
          </span>
        </div>
      </CardContent>
    </Card>
