  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (idsToDelete.length === 1) {
        await deleteJobCategory(idsToDelete[0])
      } else {
  ...
  const filteredJobCategories = useMemo(() => {
    return jobCategories.filter((jobCategory) => {
      const matchesSearch = debouncedSearchQuery === '' || 
        (jobCategory.fields.Name?.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ?? false)
      const matchesStatus = statusFilter === 'all' || jobCategory.fields.Status === statusFilter
  ...
  return (
    <AppLayout>
        ...
        <div className="border-b border-border bg-white backdrop-blur supports-[backdrop-filter]:bg-white">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-foreground tracking-tight">Job Categories</h1>
                <p className="text-sm text-muted-foreground mt-1">
                  Manage your job category database
                </p>
              </div>
              ...
              <div className="mb-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                ...
                {(searchQuery || statusFilter !== 'all') && (
                  <div className="text-sm text-muted-foreground">
                    Showing {filteredJobCategories.length} of {jobCategories.length} job category{jobCategories.length !== 1 ? 's' : ''}
                  </div>
                )}
              </div>
            )}
            ...
            {filteredJobCategories.length > 0 && (
              <div className="mb-6 flex items-center gap-4 px-4 py-3 bg-[#f9fafb] rounded-lg border border-gray-200">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={filteredJobCategories.length > 0 && filteredJobCategories.every(jc => selectedIds.has(jc.id))}
                    onChange={() => {
                      const allFilteredSelected = filteredJobCategories.every(jc => selectedIds.has(jc.id))
                  ...
                  <span className="text-sm font-medium text-foreground">
                    Select all ({filteredJobCategories.filter(jc => selectedIds.has(jc.id)).length} of {filteredJobCategories.length} selected)
                  </span>
                </label>
              </div>
            )}
              ...
              {filteredJobCategories.map((jobCategory) => (
                <Card 
                  ...
                  className={`group relative overflow-hidden rounded-xl border transition-all duration-200 ease-out ${
                    selectedIds.has(jobCategory.id) 
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      handleDeleteClick([jobCategory.id])
                    }}
                  ...
                  <CardHeader className="pb-4 pt-6 px-6">
                    <div className="flex items-start gap-3">
                      <input
                        type="checkbox"
                        checked={selectedIds.has(jobCategory.id)}
                        onChange={() => handleToggleSelect(jobCategory.id)}
                        ...
                        <div className="flex items-start gap-3 mb-3">
                          ...
                          <div className="flex-1 min-w-0 pr-8">
                            <CardTitle className="text-lg font-semibold break-words leading-tight text-foreground">
                              {jobCategory.fields.Name || 'Unnamed Product Group'}
                            </CardTitle>
                          </div>
                        </div>
                      </div>
                ...
                <CardContent className="pt-0 px-6 pb-6">
                  <div className="flex items-center justify-between mb-4 pt-4 border-t border-border">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          handleToggleActive(jobCategory.id, jobCategory.fields.Status ?? "Active")
                        }}
                        disabled={togglingIds.has(jobCategory.id)}
                  ...
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span className="text-xs">
                      Created {new Date(jobCategory.createdTime).toLocaleDateString('en-US', { 
                        month: 'short', 
                        day: 'numeric', 
                        year: 'numeric' 
                      })}
      ...
      <JobCategoryFormDialog
        ...
        onSubmit={handleCreateJobCategory}
      />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-foreground/80 mt-1 leading-relaxed">
                    {deleteConfirm.ids.length === 1
                      ? 'This action cannot be undone. The job category will be permanently deleted.'
                      : `Are you sure you want to delete ${deleteConfirm.ids.length} job categories? This action cannot be undone.`}
                  </p>
                </div>
              </div>
            </div>
