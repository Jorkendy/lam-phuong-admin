...
export function ProductGroupsPage() {
  ...
  const loadProductGroups = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load product groups"
      );
  ...
  const handleSelectAll = () => {
    if (selectedIds.size === productGroups.length) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(productGroups.map((pg) => pg.id)));
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to delete product group(s)"
      );
      setDeleteConfirm({ open: false, ids: [] });
    } finally {
      setDeleting(false);
                ...
                <div className="mb-4 flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={
                        selectedIds.size === productGroups.length &&
                        productGroups.length > 0
                      }
                          ...
                          <div className="flex items-start gap-2 mb-2">
                            ...
                            <div className="flex-1 min-w-0">
                              <CardTitle className="text-lg font-semibold break-words leading-tight">
                                {productGroup.fields.Name ||
                                  "Unnamed Product Group"}
                              </CardTitle>
                            </div>
                            <Button
                              variant="ghost"
