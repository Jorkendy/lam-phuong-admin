/**
 * Delete confirmation dialog component for Job Categories
 */
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-lg font-bold text-foreground leading-tight">
                Delete Job Category{ids.length > 1 ? 's' : ''}?
              </h2>
              <p className="text-sm text-foreground/80 mt-1 leading-relaxed">
                {ids.length === 1
                  ? 'This action cannot be undone. The job category will be permanently deleted.'
                  : `Are you sure you want to delete ${ids.length} job categories? This action cannot be undone.`}
              </p>
            </div>
          </div>
        </div>
