...
import { type JobTypeFields, generateUniqueJobTypeSlug } from "@/lib/airtable-api";

interface JobTypeFormDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: JobTypeFields) => Promise<void>;
}
...
export function JobTypeFormDialog({
  ...
  const [formData, setFormData] = useState<JobTypeFields>({
    Name: "",
  });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      try {
        uniqueSlug = await generateUniqueJobTypeSlug(formData.Name.trim());
      } catch (slugError) {
        throw new Error(
      ...
      setError(
        err instanceof Error ? err.message : "Failed to create job type"
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                Add a job type to your database
              </p>
            </div>
          </div>
        </div>
